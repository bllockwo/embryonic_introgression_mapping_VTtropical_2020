...
tmp_fil = system("ls /gpfs2/scratch/jcnunez/fst_brent/simulations/collect_COUNTS_intro", intern = T)
root = "/gpfs2/scratch/jcnunez/fst_brent/simulations/collect_COUNTS_intro"
...
foreach(i=1:length(tmp_fil),
    ...
    rownames(sim_af) = paste("intro", ith, sep = "_")
    #return(sim_af)
    save(sim_af, 
         file = paste(out,
                      paste("intro", ith, "Rdata" , sep = "."), 
                      sep ="/"))
}
