...
seqSetFilter(genofile,
             snps.dt[chr%in%c("2R")][missing<.05]$variant.id)
...
###
### --> 19115753 to 21615753
snps.dt %>%
  filter(chr == "2R" & pos > 19115753-1e6 & pos < 21615753+1e6 ) %>%
  .$SNP_id ->
  snps_of_win
...
###
fst.2R.df = foreach(i = 1:dim(comp.pairs)[1], .combine = "rbind" )%dopar%{
...
fst.2R.df %>%
  filter(samp1 %in% c("SK_0_1","VT8_0_1")) %>%
  ggplot(aes(
    x=as.numeric(pos),
    y=snp.fst,
    color=samp2
  )) + geom_point() + facet_wrap(~samp2) -> zoomfst
ggsave(zoomfst, file = "zoomfst.pdf")
